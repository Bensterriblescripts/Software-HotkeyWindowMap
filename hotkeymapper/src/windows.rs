...
use winapi::um::winuser::{EnumWindows, GetWindowTextLengthW, GetWindowTextW, 
                          IsWindowVisible, FindWindowW, GetWindowLongW, SetWindowLongW, 
                          SetWindowPos, GetSystemMetrics, ShowWindow, GWL_STYLE, 
                          SW_SHOW, SWP_FRAMECHANGED, SWP_SHOWWINDOW, WS_OVERLAPPEDWINDOW};
...
pub unsafe extern "system" fn enum_windows_callback(hwnd: HWND, lparam: LPARAM) -> BOOL {
    unsafe {
        ...
        // Convert from UTF-16 to Rust String
        let title = String::from_utf16_lossy(&buffer[0..chars_copied as usize]);
        
        // Get the reference to the tuple we passed in
...
pub fn make_focus(window_title: &str) -> Result<(), Box<dyn Error>> {
    ...
    println!("Made {} the active window", window_title);
    Ok(())
}
